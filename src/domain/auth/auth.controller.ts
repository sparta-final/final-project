...
export class AuthController {
  ...
  async KakaoLoginCallback(@CurrentUser() user: KakaoLoginUserDto, @Res() res: Response) {
    ...
    // 토큰 쿼리스트링으로 보내기
    // return res.redirect(`https://sixpack.pro/gym?at=${tokens.AccessToken}&rt=${tokens.RefreshToken}`);
    return res.redirect(`http://localhost:3000/gym?at=${tokens.AccessToken}&rt=${tokens.RefreshToken}`);
  }
